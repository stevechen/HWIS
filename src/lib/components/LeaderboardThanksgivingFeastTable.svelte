...
<section class="min-h-[calc(100vh-4rem)] {theme.section} {theme.font} p-[clamp(1rem,2vw,2rem)]">
	...
	<div
		class="mx-auto mt-[clamp(1rem,3vw,2rem)] grid max-w-[96rem] grid-cols-1 gap-[clamp(1.5rem,2.5vw,2rem)] sm:grid-cols-2 lg:grid-cols-4 xl:gap-[clamp(2rem,3vw,2.5rem)]"
	>
		{#each sorted as h (h.house)}
			{@const Crest = houseLogos[h.house as House]}
			<article
				class="rounded-2xl border bg-amber-950/60 p-[clamp(1.5rem,2vw,2.5rem)] text-center shadow-[0_20px_60px_-20px_rgba(217,119,6,0.5)] {h.rank ===
				1
			...
			>
				...
				<div
					class="mx-auto mt-2 flex size-[clamp(5rem,8vw,7rem)] items-center justify-center rounded-full bg-amber-500/10 p-[clamp(1rem,1.5vw,1.25rem)] text-amber-200 ring-2 ring-amber-400/50"
					role="img"
				...
				<p
					class="mt-2 text-[clamp(3.5rem,6vw,5.5rem)] leading-none font-black text-amber-200 drop-shadow-[0_0_20px_rgba(251,191,36,0.8)]"
				>
					{h.totalPoints}
				</p>
