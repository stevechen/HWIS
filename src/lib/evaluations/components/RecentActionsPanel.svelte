<script lang="ts">
	...
	let { panelTestId, hasRecent }: Props = $props();
	...
	// the `hasRecent` prop.
	const batchesQuery = useQuery(api.evaluations.listRecentBatches, () => (folded ? 'skip' : {}));
		...
		>
			...
			<div class="max-h-[60vh] overflow-y-auto p-2">
				{#if batchesQuery.isLoading}
					<p class="text-muted-foreground p-2 text-xs">Loading recent actions...</p>
				{:else if actionable.length > 0}
					...
					{/each}
				{:else}
					<p class="text-muted-foreground p-2 text-xs">No recent actions to fix.</p>
				{/if}
			</div>
		</div>
	{/if}
