<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			error = err.message || 'Failed to save evaluations';
		} finally {
			loading = false;
		}
	}
	...
	function getCategoryLabel(id: string): string {
		return categoriesQuery.data?.find((c) => c._id === id)?.name || '';
	}
</script>
	...
	<div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
		<Card.Root>
			...
			<Card.Content>
				...
				<div class="bg-muted max-h-[300px] overflow-y-auto rounded-md border">
				...
				<div class="mb-5">
					<label class="mb-2 block text-sm font-medium">
						...
						<textarea
							...
							class="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring mt-1 flex min-h-[80px] w-full resize-none rounded-md border px-3 py-2 text-sm focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50"
							rows="4"
						></textarea>
					</label>
				</div>
