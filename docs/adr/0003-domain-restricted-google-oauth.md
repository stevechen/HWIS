...
- **Students have no user profile.** Their identity is derived from their email on every read: the address must match `s{studentId}@std.hwhs.tc.edu.tw`, and the resulting student record's **Enrolled** status gates access to their own evaluations. Access is enforced in-app (in the `viewer` query and the anonymous evaluations query), not at the auth hook.
